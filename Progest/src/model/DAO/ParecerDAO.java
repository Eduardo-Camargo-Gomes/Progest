...
public class ParecerDAO {
    ...
    List<ParecerModel> listaIdsParecer = new ArrayList<>();
    ...
    public void salvarParecer(ParecerModel parecerASalvar) {
        ...
        String sql = "INSERT INTO parecer (assunto_parecer, interessado_parecer, "
                + "matricula_aluno, texto, locall, data_ocorrido, concluido, dataAcesso, dataModificacao, descricao)"
                + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
// instancia um objeto model e passa por parametro o id para consultar
    ...
    public List<ParecerModel> listaIdsParecer(String tipoOrdenacao) throws SQLException {

        String sql = null;
        if (tipoOrdenacao.equals("Mais recentes primeiro")) {
            listaIdsParecer.clear();
            sql = "select numero_parecer, descricao from parecer order by numero_parecer desc";
        }// fim if 
        else if (tipoOrdenacao.equals("Já concluídos")) {
            listaIdsParecer.clear();
            sql = "select numero_parecer, descricao from parecer where concluido = true order by numero_parecer desc";
        }// fim else if
        else if (tipoOrdenacao.equals("Últimos acessados")) {
            listaIdsParecer.clear();
            sql = "select numero_parecer, descricao from parecer order by dataAcesso desc;";
        }// fim else if
        else if (tipoOrdenacao.equals("Última modificação")) {
            listaIdsParecer.clear();
            sql = "select numero_parecer, descricao from parecer order by dataModificacao desc;";
        }// fim else if
        else if (tipoOrdenacao.equals("Mais antigos primeiro")) {
            listaIdsParecer.clear();
            sql = "select numero_parecer, descricao from parecer order by numero_parecer";
        }// fim else if
        ...
        while (resultSet.next()) {
            ...
            int numParecer = resultSet.getInt("numero_parecer");

            ParecerModel parecer = new ParecerModel(numParecer, descricao);
            listaIdsParecer.add(parecer);
        }// fim while
    ...
    public List<Integer> passarAnterior() throws SQLException {
        ...
        public String alterarDescricao(String descricao, int numParecer){
         ...
         try {
            ...
            String sql = "update parecer set descricao = ?  where numero_parecer = ?";
